...
def ValidateOutVideoteca(request):
    if request.method == 'POST':
        ...
        cursor.execute("SELECT matricula, nombres, apellido1, apellido2 FROM people_person WHERE matricula IN %s", (tuple("H230717"),))
        # if matriculas_list:
        # users_data = cursor.fetchall()
        # else:
        #     users_data = []
...
def ValidateOutVideoteca(request):
    if request.method == 'POST':
        ...
        if not usuario or not codigoBarras:
            ...
            cursor.execute("SELECT matricula FROM people_person WHERE matricula = %s", (usuario,))
            user_exists = cursor.fetchone()

            if not user_exists:
                registro_data = {
                    "error": True,
                    "errorMessage": "La matrícula ingresada, no pertenece a ningún usuario"
                }
            else:
                try:
                    maestroCinta = MaestroCintas.objects.get(pk=codigoBarras)
